...
class PS4Controller():
    ...
    def listen_axis(self, robot_state, arm, actuation_function_axis):
        ...
        self.task_list = ['Home', 'Drinking', 'Before_Destruct','Shelf','Before_Destruct']
        self.task_list = ['Home', 'Drinking','Shelf','Home','Before_Destruct']
                ...
                for event in pygame.event.get():
                    if event.type == pygame.JOYAXISMOTION:
                        ...
                        if self.joy == 1:
                            ...
                            if x_0 > default or x_0 < -default: # Left or Right
                                act = True
                                if x_0 < default: # Left
                            ...
                            elif y_1 > default or y_1 < -default: # Forward or Backward
                                act = True
                                if y_1 < default: # Forward
                    ...
                    if actuation_function_axis is not None and \
                        ...
                        '''
                        In order to prevent double actions & still pay attention to buttons 4 and 5:
                        '''
                        if time_tmp > 0.144 or notice :        # no nengo
                        ##if time_tmp > 0.352 or notice :      # Direct
                        ##if time_tmp > 0.366 or notice :      # LIF OPNEU
                        ##if time_tmp > 0.479 or notice :      # LIF ALYN
                        ##if time_tmp > self.get_loihi_time() or notice :  # LIF LOIHI  
                            ...
                            actuation_function_axis(self, robot_state, act, self.axis_direction ,self.buttons, arm)
